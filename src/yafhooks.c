...
typedef void (*yfHookFlowAlloc_fn)(void ** yfHookConext, yfFlow_t *flow, void *yfctx);
...
typedef void (*yfHookSetPluginOpt_fn)(const char * pluginOpt, void *yfctx);
typedef void (*yfHookSetPluginConf_fn)(const char * pluginConf, void **yfctx);
#if YAF_ENABLE_APPLABEL
...
#endif
typedef gboolean (*yfHookValidateFlowTab_fn)(void *yfctx,
    ...
    yfL2Info_t * l2info)
{
    ...
    for (loop = 0; loop < yaf_hooked; loop++) {
        ...
        if (FALSE == (pluginIndex->ufptr.funcPtrs.hookPacket) (key, pkt,
                                                       caplen, iplen, tcpinfo,
                                                               l2info))
        {
            return FALSE;
        }
    ...
    yfL2Info_t * l2info)
{
    ...
    for (loop=0; loop < yaf_hooked; loop++) {
        ...
        (pluginIndex->ufptr.funcPtrs.flowPacket)((flow->hfctx)[loop],
                                                 flow, val, pkt, caplen, iplen,
                                                 tcpinfo, l2info);
        pluginIndex = pluginIndex->next;
}
    return;
    ...
    uint16_t        udp_uniflow_port)
{
    ...
    for (loop = 0; loop < hooked; loop++) {
       ...
       if (FALSE == pluginIndex->ufptr.funcPtrs.validateFlowTab(yfctx[loop],
                                                        max_payload,
                                                        uniflow,
                                                        silkmode,
                                                        applabelmode,
    ...
    void     **yfctx)
{
    ...
    for (loop=0; loop < yaf_hooked; loop++) {
        ...
        (pluginIndex->ufptr.funcPtrs.flowAlloc) (&((flow->hfctx)[loop]),
                                                 flow, yfctx[loop]);
        pluginIndex = pluginIndex->next;
    }
    return;
 ...
 * @param hfctx context for yaf plugins
